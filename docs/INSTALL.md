...
O objetivo deste documento é descrever o procedimento para realizar a INSTALAÇÃO INICIAL do Módulo de Resposta à plataforma Gov.br (**mod-sei-resposta**) no Sistema Eletrônico de Informações (SEI).
...
A utilização deste módulo adicionará nova funcionalidade ao SEI, permitindo:
 ...
 - Solicitar ajuste ou complementação à solicitação;
 - Enviar resultado à processos administrativos abertos por meio do Protocolo GOV.BR.

Este documento está estruturado nas seguintes seções:

1. **[Instalação](#instalação)**:
    Procedimentos destinados à Equipe Técnica responsável pela instalação do módulo nos servidores web e atualização do banco de dados.

2. **[Suporte](#suporte)**:
    Canal de comunicação para resolver problemas ou tirar dúvidas sobre o módulo.
...
A atualização realizada no SIP não cria nenhuma tabela específica para o módulo, apenas é aplicada a criação os recursos, permissões e menus de sistema utilizados pelo **mod-sei-resposta**. Todos os novos recursos criados possuem o prefixo **md_resposta_** para fácil localização pelas funcionalidades de gerenciamento de recursos do SIP.
...
Esta seção descreve os passos de configuração do Módulo no SEI. Todos os itens descritos nesta seção são destinados aos Administradores do sistema SEI da instituição, responsáveis pela alteração de configurações gerais do sistema através do menu de administração do SEI.


### 2.1. Configurar os parâmetros do Módulo de Resposta ao Protocolo GOV.BR
Acesse a funcionalidade **[SEI > Administração > Módulo de Resposta ao Protocolo GOV.BR > Parâmetros de Configuração]** para configurar os parâmetros de funcionamento do módulo:  

#### Sistema
*Identificação do sistema que terá acesso ao webservice de resposta.*

#### Tipo de Documento para Solicitação de Ajuste ou Complementação:
*Identificação do Tipo de Documento que terá acesso a funcionalidade de solicitação de ajuste ou complepemntação à processos administrativos abertos por meio do Protocolo GOV.BR.*  

#### Tipo de Documento para Resultado:
*Identificação do Tipo de Documento que será utilizado para envio do Resultado através da plataforma.*  
...
Como o envio é realizado de forma automática, o sistema precisa atribuir um Tipo de Documento padrão. Com isto, sugerimos a criação de um tipo de documento específico para estes processos, permitindo a fácil identificação e reclassificação, caso necessário. Segue abaixo um exemplo de Tipo de Documento que pode ser criado para esta situação:
    ...
    Nome: Resposta pelo Protocolo GOV.BR
    Descrição: Indicado nos Parâmetros para o envio de Resposta pelo Protocolo GOV.BR
...
> Favor verificar noS [manuais do PGOV](https://manuais.processoeletronico.gov.br/pt-br/latest/PROTOCOLO.GOV.BR/index.html e https://manuais.processoeletronico.gov.br/pt-br/latest/PROTOCOLO.GOV.BR/MTO_ProtocoloGOV.BR_Sydle/index.html) outros requisitos necessários.
...
- [Portal de Atendimento (PEN): Canal de Atendimento](https://portaldeservicos.gestao.gov.br/) - Módulo Resposta
