...
XML;
     ...
     * Equivalente ao metodo Python test_06EnviarRespostaAjusteSemDocumentoAnexo (linha 92 em diante).
     *
    ...
    public function testEnviarRespostaAjusteSemDocumento($procedimentoFormatado)
    {
        ...
        $this->frame(0);
        $this->waitUntil(function () {
            return $this->elementoVisivel('txaMensagem', 'id');
        }, 30000);
        $this->byId('txaMensagem')->click();
        $this->byId('txaMensagem')->value('teste');
        // Envio Parcial (ajuste/complementacao) sem selecionar documento de anexo (sem imgInfraCheck)
