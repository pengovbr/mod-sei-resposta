 ...
 *  - testEnviarRespostaAjuste: executa o cenario de UI (Selenium) e valida o envio da resposta
 *    para ajuste/complementacao (envio parcial).
 */
...
XML;
    ...
    /**
     * Etapa 2: cenario de validacao do envio da resposta para ajuste/complementacao (envio parcial).
     * Equivalente ao metodo Python test_01EnviarRespostaAjuste (linha 92 em diante).
     *
    ...
    public function testEnviarRespostaAjuste($procedimentoFormatado)
    {
        ...
        $this->frame(0);
        $this->waitUntil(function () {
            return $this->elementoVisivel("//*[@id='lblMensagem']");
        }, 30000);
        $this->byId('txaMensagem')->click();
        $this->byId('txaMensagem')->value('teste');
        $this->byId('imgInfraCheck')->click();
        // Diferente do envio definitivo, aqui marca-se o envio Parcial (ajuste/complementacao)
        $this->byId('lblParcial')->click();
        ...
        $this->frame(0);
        $this->waitUntil(function () {
            return $this->elementoVisivel("//span[contains(.,'Resposta pelo Protocolo Digital')]");
        }, 30000);
        $this->assertGreaterThan(
            0,
            count($this->elements($this->using('xpath')->value("//span[contains(.,'Resposta pelo Protocolo Digital')]"))),
            'Documento "Resposta pelo Protocolo Digital" nao encontrado na arvore do processo.'
        );
        $this->byXPath("//span[contains(.,'Resposta pelo Protocolo Digital')]")->click();
        ...
        $this->frame(0);
        $this->waitUntil(function () {
            return $this->elementoVisivel("//label[contains(.,'Resposta pelo Protocolo Digital')]");
        }, 30000);
        ...
        $this->assertGreaterThan(
            0,
            count($this->elements($this->using('xpath')->value("//label[contains(.,'Resposta pelo Protocolo Digital')]"))),
        ...
        $this->frame(1);
        $this->waitUntil(function () {
            return $this->elementoVisivel("//img[@alt='Consultar/Alterar Processo']");
        }, 30000);
        $this->assertCount(
            0,
            $this->elements($this->using('xpath')->value("//img[@alt='Enviar Resposta']")),
